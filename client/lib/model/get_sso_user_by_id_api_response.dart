...
class GetSSOUserByIdAPIResponse {
  ...
  static GetSSOUserByIdAPIResponse? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'status'), 'Required key "GetSSOUserByIdAPIResponse[status]" is missing from JSON.');
        assert(json[r'status'] != null, 'Required key "GetSSOUserByIdAPIResponse[status]" has a null value in JSON.');
        return true;
      }());
