...
class GetUser200Response {
  ...
  @override
  String toString() => 'GetUser200Response[status=$status, user=$user, reason=$reason, code=$code, secondaryCode=$secondaryCode, bannedUntil=$bannedUntil, maxCharacterLength=$maxCharacterLength, translatedError=$translatedError, customConfig=$customConfig]';
  ...
  static GetUser200Response? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'status'), 'Required key "GetUser200Response[status]" is missing from JSON.');
        assert(json[r'status'] != null, 'Required key "GetUser200Response[status]" has a null value in JSON.');
        assert(json.containsKey(r'user'), 'Required key "GetUser200Response[user]" is missing from JSON.');
        assert(json[r'user'] != null, 'Required key "GetUser200Response[user]" has a null value in JSON.');
        assert(json.containsKey(r'reason'), 'Required key "GetUser200Response[reason]" is missing from JSON.');
        assert(json[r'reason'] != null, 'Required key "GetUser200Response[reason]" has a null value in JSON.');
        assert(json.containsKey(r'code'), 'Required key "GetUser200Response[code]" is missing from JSON.');
        assert(json[r'code'] != null, 'Required key "GetUser200Response[code]" has a null value in JSON.');
        return true;
      }());
