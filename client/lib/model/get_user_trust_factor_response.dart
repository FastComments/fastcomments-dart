...
class GetUserTrustFactorResponse {
  ...
  static GetUserTrustFactorResponse? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'status'), 'Required key "GetUserTrustFactorResponse[status]" is missing from JSON.');
        assert(json[r'status'] != null, 'Required key "GetUserTrustFactorResponse[status]" has a null value in JSON.');
        return true;
      }());
