...
class GetSSOUserByEmailAPIResponse {
  ...
  static GetSSOUserByEmailAPIResponse? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'status'), 'Required key "GetSSOUserByEmailAPIResponse[status]" is missing from JSON.');
        assert(json[r'status'] != null, 'Required key "GetSSOUserByEmailAPIResponse[status]" has a null value in JSON.');
        return true;
      }());
