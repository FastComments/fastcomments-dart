...
class PatchDomainConfigParams {
  /// Returns a new [PatchDomainConfigParams] instance.
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is PatchDomainConfigParams &&
    other.domain == domain &&
  ...
  @override
  String toString() => 'PatchDomainConfigParams[domain=$domain, emailFromName=$emailFromName, emailFromEmail=$emailFromEmail, logoSrc=$logoSrc, logoSrc100px=$logoSrc100px, footerUnsubscribeURL=$footerUnsubscribeURL, emailHeaders=$emailHeaders]';
  ...
  /// Returns a new [PatchDomainConfigParams] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static PatchDomainConfigParams? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "PatchDomainConfigParams[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "PatchDomainConfigParams[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<PatchDomainConfigParams> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <PatchDomainConfigParams>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = PatchDomainConfigParams.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, PatchDomainConfigParams> mapFromJson(dynamic json) {
    final map = <String, PatchDomainConfigParams>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = PatchDomainConfigParams.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of PatchDomainConfigParams-objects as value to a dart map
  static Map<String, List<PatchDomainConfigParams>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<PatchDomainConfigParams>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = PatchDomainConfigParams.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
