...
class SaveCommentsBulkResponse {
  /// Returns a new [SaveCommentsBulkResponse] instance.
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is SaveCommentsBulkResponse &&
    other.status == status &&
  ...
  @override
  String toString() => 'SaveCommentsBulkResponse[status=$status, comment=$comment, user=$user, moduleData=$moduleData, reason=$reason, code=$code, secondaryCode=$secondaryCode, bannedUntil=$bannedUntil, maxCharacterLength=$maxCharacterLength, translatedError=$translatedError, customConfig=$customConfig]';
  ...
  /// Returns a new [SaveCommentsBulkResponse] instance and imports its values from
  ...
  static SaveCommentsBulkResponse? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'status'), 'Required key "SaveCommentsBulkResponse[status]" is missing from JSON.');
        assert(json[r'status'] != null, 'Required key "SaveCommentsBulkResponse[status]" has a null value in JSON.');
        return true;
      }());
  ...
  static List<SaveCommentsBulkResponse> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <SaveCommentsBulkResponse>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = SaveCommentsBulkResponse.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, SaveCommentsBulkResponse> mapFromJson(dynamic json) {
    final map = <String, SaveCommentsBulkResponse>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = SaveCommentsBulkResponse.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of SaveCommentsBulkResponse-objects as value to a dart map
  static Map<String, List<SaveCommentsBulkResponse>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<SaveCommentsBulkResponse>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = SaveCommentsBulkResponse.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
